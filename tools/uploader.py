...
class Uploader:
    ...
    @staticmethod
    def get_file_md5_sync(file_name: str, each_read_size: int = 10 * 2**20) -> str:
        """
        获取文件的md5值

        - param file_name               :{str} {description}
        - param each_read_size=10*2**20 :{int} 每次写入文件的数据大小，第一个10表示: 10 MiB
        ...
        with open(file_name, "rb") as file:
            finished = True
            while finished:
                data = file.read(each_read_size)
                md5_obj.update(data)

                if len(data) != each_read_size:
                    break
    ...
    async def get_file_md5(file_name: str) -> str:
        ...
        async with open(file_name, "rb") as file:
            ...
            finished = True
            while finished:
                data = await file.read(each_read_size)
                md5_obj.update(data)
    ...
    @staticmethod
    async def stream_file_sync(
        file, output_path: str, each_read_size: int = None
    ) -> bool:
        """
        大文件上传，以流的形式按量读取保存文件，默10m
        """
        try:
            if not each_read_size:
                # 每次写入文件的数据大小，这里代表10 MiB
                each_read_size = 10 * 2**20

            async with aiofiles.open(output_path, "wb") as f:  # 分批写入数据
                # 从网络文件流分批读取数据到 b'',再写入文件
                finished = True
                while finished:
                    data = await file.read(each_read_size)
