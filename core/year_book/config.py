...
ColNames: TypeAlias = str  # "CHMR" 等，以字符串拼接好的方式
ReaplceVal: TypeAlias = str
SearchVal: TypeAlias = str


class ABBYYearBookExcelFilterSettings(BaseModel):
    fix_24_to_new_day: bool = True  # 部分年鉴会使用24作为小时单位，修复后改位置变成0，日数据添加1
    fix_24_col_names: list[str] | str = "CHMR"
    word_strip: str  # 一些在数据前后的多余符号
    word_wipe: list[str] | str  # 一些需要擦除的字符
    ...
    word_report: str | list[str]  # 需要报告的错误字符，3% 可能会是396 等
    word_replace_in_col: dict[
        ColNames, dict[ReaplceVal, SearchVal]
    ]  # 对不同的列进行不同的过滤，时间列、数据列等


DEFAULT_SETTINGS = {
    "word_report": "%?~",  # 一些必须要肉眼校对的数据，会通过打印数据具体位置
    "word_strip": '. y、",/',  # 一些可能会出现在数据前后的多余符号 -符号需要保留
    "word_wipe": "'·-一 •■\\/《’^",  # 一些需要擦除的字符
